...
coder = Autoencoder.load(model, "ordered", layer=18, expansion=16, alpha=0.1, tags=['v2']).half().eval()
ordered = incremental(coder)

# Get reconstructions for vanilla coder
coder = Autoencoder.load(model, "vanilla", layer=18, expansion=16, alpha=0.1, tags=[]).half().eval()
