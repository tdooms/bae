...
hooked = Hooked(model, module=Input(model.model.layers[autoencoder.config.layer]))
...
run = wandb.init(project=project, name=name, config=autoencoder.config) if project else None
...
for step, (acts, batch) in progress:
    ...
    progress.set_description(f"loss = {float(loss):.3f} | mse = {float(metrics['mse']):.3f}")
    
    run.log(metrics, step=step, commit=True) if project else None
...
run.finish() if project else None
# %%
