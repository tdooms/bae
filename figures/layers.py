...
results = []
for run in tqdm(chain(combined_sweep, ordered_sweep, mixed_sweep)):
    history = run.history()
...
# %%
fig = px.bar(df, y='mse', x="layer", color="kind", template="plotly_white", barmode='group', color_discrete_map=COLORS)
fig.update_xaxes(title="<b>Layer</b>", tickvals=df['layer'].unique())
