...
coders = [Autoencoder.load(model, "mixed", layer=18, expansion=16, alpha=0.2, tags=[f"checkpoint-{i*1024}"]).half() for i in tqdm(range(17, 33))]
...
def similarities(a, b, reg=20.0):
    ...
    return dict(norm=norm.item(), partial=1.0, sparsity=1.0)
    
    # # Compute the mapping between the feature bases
    # mapping = a.down.T @ matrix @ b.down
    
    # # Compute the best permutation of the kernel matrix using the Hungarian algorithm
    # row_ind, col_ind = linear_sum_assignment(mapping.abs().cpu().numpy(), maximize=True)
    # partial = mapping[row_ind, col_ind].pow(2).sum() / norm
    
    # # Compute the sparsity, energy ratio, and procrustes error
    # sparsity = hoyer(mapping).mean()
