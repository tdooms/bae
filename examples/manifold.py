...
from utils.functions import generalized_participation_ratio
...
# %%
# coder = Autoencoder.load(model, layer=18, expansion=16, root='weights/asparse-sweep', tags=['10']).half()
coder = Autoencoder.load(model, layer=18, expansion=16, root='weights/asparse-sweep', tags=['2']).half()
...
gpr = generalized_participation_ratio(g)
...
manifold = Manifold(dataset, coder.hooked, tokenizer, density, max_steps=2**6)
manifold.spectrum()
# %%
manifold(k=10_000)
# %%