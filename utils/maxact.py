from autoencoders.utils import Input, Hooked, IndexSampler
...
class MaxAct:
    def __init__(self, model, autoencoder, dataset, tokenizer, batches=16, batch_size=32):
        ...
        self.sampler = IndexSampler(lambda **batch: autoencoder(hooked(**batch)), dataset, device=model.device)
        self.tokenizer = tokenizer
        
        iterator = map(lambda i: self.sampler[i*batch_size:(i+1)*batch_size][0], range(batches))
    ...
    def color_inputs(self, inds, feature, view=range(-10, 10), dark=True, largest=True):
        features, batch = self.sampler[inds]
